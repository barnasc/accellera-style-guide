...
To include a figure the container `{% raw %}{% include figure %}{% endraw %}` should be used. The figure reference, caption and style can be added to this container. The image can be embedded in this container using the `images` property, see the example below. To maintain a high quality and high resolution, it is recommended to use vector graphics in Scalable Vector Graphics (SVG) format. The size of the image can be specified using the `image-height` property, where the value represents the line height.

{% raw %}
```
{% include figure
   reference="Figure 1"
   caption="Accellera logo in Scalable Vector Graphics (SVG) format"
   alt-text=""
   class="fixed"
...
[^fn]: And here is the informative footnote description including a link to [Chapter 1](01.md).
```
...
<!--As Jekyll replace a Markdown link by a HTML variant, we use a HTML and obfuscate the link -->
